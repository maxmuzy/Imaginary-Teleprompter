...
- **Voice Control System (v29.4)**:
    ...
    - **Local-First SEARCHING (v29.7)**: Uses ONLY `lastLockedReadableIndex` (not `currentElementIndex`) to decide local vs global search. When `lastLockedReadableIndex >= 0`, searches +/-50 element window first; otherwise goes directly to global search. `finalizarBusca()` helper handles LOCKED transition.
