...
- **Voice Control System (v29.4)**:
    ...
    - **Local-First SEARCHING (v29.7)**: Uses ONLY `lastLockedReadableIndex` (not `currentElementIndex`) to decide local vs global search. When `lastLockedReadableIndex >= 0`, searches +/-50 element window first; otherwise goes directly to global search. `finalizarBusca()` helper handles LOCKED transition. `posicionarNoInicio()` does NOT set `lastLockedReadableIndex` - ensures first search is always GLOBAL.
    - **Silence Detection (v29.7)**: Centralized `armSilenceTimer()` function with `silenceSuppressed` flag. Timer (800ms) only armed when: no existing timer, not suppressed, isActive, !isPaused, and state LOCKED. `softStop()` sets suppression flag; `softResume()` clears it and re-arms timer.
    - **Resume Impulse (v29.7)**: Uses `pauseStartTimestamp` set in `softStop()` to calculate actual pause duration (`pausaEfetiva`). Velocity kick (v=3) only applied after pauses >1.5s; cleared in `softResume()`.
    - **Pause Repositioning (v29.7)**: `reposicionarParaProximoLegivel()` visually scrolls to next readable element without altering `currentElementIndex` or `lastLockedReadableIndex`, preserving matching state.
